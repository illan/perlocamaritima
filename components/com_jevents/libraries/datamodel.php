 ...
 *
 * @version     $Id: datamodel.php 1869 2010-12-17 11:54:52Z geraint $
 * @package     JEvents
...
class JEventsDataModel {
	...
	function getCalendarData( $year, $month, $day , $short=false, $veryshort = false){
		...
		for ($w=0;$w<6 && $w*7<count($data["dates"]);$w++){
			...
			// get week number from second weekday + 1 hour to avoid confusion with week start sunday
			//$x = strftime("%V %Y-%m-%d %H:%M:%S",$date);
			//$y = strftime("%V %Y-%m-%d %H:%M:%S",$date+90000);
			$week = intval(JEV_CommonFunctions::jev_strftime("%V",$date+90000));
